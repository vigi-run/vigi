...
test.describe('Invitation Flow', () => {
  test('Invite Member and Accept as New User', async ({ page, browser }) => {
    ...
    await page.getByPlaceholder('colleague@example.com').fill(inviteeEmail);
    ...
    // Invitee Context: Accept invitation as a new user (UNAUTHENTICATED - no storage state)
    ...
    // Verify invitation page loaded (check Create Account button is visible)
    await expect(inviteePage.getByRole('button', { name: /Create Account/i })).toBeVisible();
    ...
    // Should redirect to the organization dashboard (wait longer for redirect)
    await inviteePage.waitForURL('**/monitors', { timeout: 30000 });
    await expect(inviteePage).toHaveURL(/.*\/monitors$/);

    // Verify access to organization (URL check is sufficient, page may show 'No monitors found' when empty)
    await expect(inviteePage).toHaveURL(/.*\/monitors$/, { timeout: 10000 });
